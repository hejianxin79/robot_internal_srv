...
class RobotInternalServer:
    ...
    def server_cb(self,req):
        ...
        if result["method"] == "start_record_motion":  # 开始录制
            ...
            self.data["code"] = 0
            self.data["msg"] = "OK"
            self.ros_bag_controller.start_recording(file_name=file_name, topic=self.joint_topic)
        ...
        if result["method"] == "stop_record_motion": # 停止录制
            ...
            self.data["code"] = 0
            self.data["msg"] = "OK"
            self.ros_bag_controller.stop_recording()
            resp = internalResponse(json.dumps(self.data, ensure_ascii=False))
        ...
        if result["method"] == "start_play_motion": # 开始播放
            ...
            self.data["msg"] = "OK"
            resp = internalResponse(json.dumps(self.data, ensure_ascii=False))
            file = self.parent_dir+"/"+str(file_name)+".bag"
            self.ros_bag_controller.start_playing(bag_file=file)
        ...
        if result["method"] == "stop_play_motion": # 停止播放
            ...
            self.data["code"] = 0
            self.data["msg"] = "OK"
            self.ros_bag_controller.stop_playing()
